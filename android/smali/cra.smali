.class public final Lcra;
.super Ljava/lang/Object;
.source "SourceFile"

# interfaces
.implements Loqa;
.implements Lum1;


# instance fields
.field public final X:Lje7;

.field public final Y:Lje7;

.field public final Z:Lkhe;

.field public final a:Ljr1;

.field public final b:Lso1;

.field public final c:Lld1;

.field public final o:Lje7;

.field public final s0:Ljava/util/concurrent/atomic/AtomicReference;

.field public final t0:Lkld;

.field public u0:Lvxd;

.field public v0:Lvxd;

.field public final w0:Lw7c;


# direct methods
.method public constructor <init>(Lje7;Lje7;Ljr1;Lso1;Lld1;Lje7;Lje7;)V
    .locals 0

    invoke-direct {p0}, Ljava/lang/Object;-><init>()V

    iput-object p3, p0, Lcra;->a:Ljr1;

    iput-object p4, p0, Lcra;->b:Lso1;

    iput-object p5, p0, Lcra;->c:Lld1;

    iput-object p1, p0, Lcra;->o:Lje7;

    iput-object p7, p0, Lcra;->X:Lje7;

    iput-object p2, p0, Lcra;->Y:Lje7;

    new-instance p1, Lzj7;

    const/16 p2, 0x18

    invoke-direct {p1, p2, p0}, Lzj7;-><init>(ILjava/lang/Object;)V

    new-instance p2, Lkhe;

    invoke-direct {p2, p1}, Lkhe;-><init>(Lk56;)V

    iput-object p2, p0, Lcra;->Z:Lkhe;

    new-instance p1, Ljava/util/concurrent/atomic/AtomicReference;

    new-instance p2, Lpqa;

    sget-object p4, Lfqa;->c:Lfqa;

    invoke-direct {p2, p4}, Lpqa;-><init>(Lfqa;)V

    invoke-direct {p1, p2}, Ljava/util/concurrent/atomic/AtomicReference;-><init>(Ljava/lang/Object;)V

    iput-object p1, p0, Lcra;->s0:Ljava/util/concurrent/atomic/AtomicReference;

    const/4 p2, 0x2

    const/4 p4, 0x1

    invoke-static {p4, p4, p2}, Llld;->a(III)Lkld;

    move-result-object p2

    iput-object p2, p0, Lcra;->t0:Lkld;

    invoke-interface {p6}, Lje7;->getValue()Ljava/lang/Object;

    move-result-object p4

    check-cast p4, Los1;

    invoke-virtual {p4, p0}, Los1;->d(Lum1;)V

    new-instance p4, Ly03;

    const/4 p5, 0x3

    const/4 p6, 0x0

    const/4 p7, 0x6

    invoke-direct {p4, p5, p6, p7}, Ly03;-><init>(ILkotlin/coroutines/Continuation;I)V

    invoke-static {p2, p4}, Lod2;->d0(Lmn5;Lc66;)Lx32;

    move-result-object p2

    new-instance p4, Lt03;

    const/16 p5, 0xb

    invoke-direct {p4, p2, p5}, Lt03;-><init>(Lmn5;I)V

    new-instance p2, Lm58;

    const/16 p5, 0x14

    invoke-direct {p2, p4, p5}, Lm58;-><init>(Lmn5;I)V

    new-instance p4, Luqa;

    const/4 p5, 0x1

    invoke-direct {p4, p2, p0, p5}, Luqa;-><init>(Lmn5;Lcra;I)V

    invoke-static {p4}, Lod2;->x(Lmn5;)Lmn5;

    move-result-object p2

    new-instance p4, Luqa;

    const/4 p5, 0x2

    invoke-direct {p4, p2, p0, p5}, Luqa;-><init>(Lmn5;Lcra;I)V

    sget-object p2, Lwld;->b:Lnd2;

    invoke-virtual {p1}, Ljava/util/concurrent/atomic/AtomicReference;->get()Ljava/lang/Object;

    move-result-object p1

    invoke-static {p4, p3, p2, p1}, Lod2;->X(Lmn5;Lsx3;Lxld;Ljava/lang/Object;)Lw7c;

    move-result-object p1

    iput-object p1, p0, Lcra;->w0:Lw7c;

    return-void
.end method


# virtual methods
.method public final b()V
    .locals 3

    iget-object v0, p0, Lcra;->o:Lje7;

    invoke-interface {v0}, Lje7;->getValue()Ljava/lang/Object;

    move-result-object v0

    check-cast v0, Lfw3;

    invoke-virtual {v0}, Lfw3;->a()Lru/ok/android/externcalls/sdk/Conversation;

    move-result-object v0

    const/4 v1, 0x0

    if-eqz v0, :cond_0

    invoke-interface {v0}, Lru/ok/android/externcalls/sdk/Conversation;->getParticipantStatesManager()Lru/ok/android/externcalls/sdk/participant/state/ParticipantStatesManager;

    move-result-object v0

    goto :goto_0

    :cond_0
    move-object v0, v1

    :goto_0
    if-eqz v0, :cond_1

    iget-object v2, p0, Lcra;->Z:Lkhe;

    invoke-virtual {v2}, Lkhe;->getValue()Ljava/lang/Object;

    move-result-object v2

    check-cast v2, Lru/ok/android/externcalls/sdk/participant/state/ParticipantStatesManager$Listener;

    invoke-interface {v0, v2}, Lru/ok/android/externcalls/sdk/participant/state/ParticipantStatesManager;->removeHandListener(Lru/ok/android/externcalls/sdk/participant/state/ParticipantStatesManager$Listener;)V

    :cond_1
    iget-object v0, p0, Lcra;->u0:Lvxd;

    if-eqz v0, :cond_2

    invoke-virtual {v0, v1}, Lz87;->cancel(Ljava/util/concurrent/CancellationException;)V

    :cond_2
    iput-object v1, p0, Lcra;->u0:Lvxd;

    iget-object v0, p0, Lcra;->v0:Lvxd;

    if-eqz v0, :cond_3

    invoke-virtual {v0, v1}, Lz87;->cancel(Ljava/util/concurrent/CancellationException;)V

    :cond_3
    iput-object v1, p0, Lcra;->v0:Lvxd;

    iget-object v0, p0, Lcra;->s0:Ljava/util/concurrent/atomic/AtomicReference;

    invoke-virtual {v0}, Ljava/util/concurrent/atomic/AtomicReference;->get()Ljava/lang/Object;

    move-result-object v0

    if-eqz v0, :cond_4

    new-instance v0, Lpqa;

    invoke-virtual {p0}, Lcra;->c()Lfqa;

    move-result-object v1

    invoke-direct {v0, v1}, Lpqa;-><init>(Lfqa;)V

    invoke-virtual {p0, v0}, Lcra;->e(Lpqa;)V

    :cond_4
    return-void
.end method

.method public final c()Lfqa;
    .locals 0

    iget-object p0, p0, Lcra;->s0:Ljava/util/concurrent/atomic/AtomicReference;

    invoke-virtual {p0}, Ljava/util/concurrent/atomic/AtomicReference;->get()Ljava/lang/Object;

    move-result-object p0

    check-cast p0, Lpqa;

    iget-object p0, p0, Lpqa;->a:Lfqa;

    return-object p0
.end method

.method public final d()V
    .locals 1

    iget-object v0, p0, Lcra;->t0:Lkld;

    iget-object p0, p0, Lcra;->o:Lje7;

    invoke-interface {p0}, Lje7;->getValue()Ljava/lang/Object;

    move-result-object p0

    check-cast p0, Lfw3;

    invoke-virtual {p0}, Lfw3;->a()Lru/ok/android/externcalls/sdk/Conversation;

    move-result-object p0

    invoke-virtual {v0, p0}, Lkld;->g(Ljava/lang/Object;)Z

    return-void
.end method

.method public final e(Lpqa;)V
    .locals 2

    iget-object p0, p0, Lcra;->s0:Ljava/util/concurrent/atomic/AtomicReference;

    invoke-virtual {p0}, Ljava/util/concurrent/atomic/AtomicReference;->get()Ljava/lang/Object;

    move-result-object v0

    :cond_0
    invoke-virtual {p0, v0, p1}, Ljava/util/concurrent/atomic/AtomicReference;->compareAndSet(Ljava/lang/Object;Ljava/lang/Object;)Z

    move-result v1

    if-eqz v1, :cond_1

    goto :goto_0

    :cond_1
    invoke-virtual {p0}, Ljava/util/concurrent/atomic/AtomicReference;->get()Ljava/lang/Object;

    move-result-object v1

    if-eq v1, v0, :cond_0

    :goto_0
    return-void
.end method

.method public final onCallEnded(Lru/ok/android/externcalls/sdk/events/end/ConversationEndInfo;)V
    .locals 0

    invoke-virtual {p0}, Lcra;->b()V

    return-void
.end method

.method public final onCallParticipantsNetworkStatusChanged(Ljava/util/List;)V
    .locals 0

    invoke-virtual {p0}, Lcra;->d()V

    return-void
.end method

.method public final onDestroyed(Lru/ok/android/externcalls/sdk/events/destroy/ConversationDestroyedInfo;)V
    .locals 0

    invoke-virtual {p0}, Lcra;->b()V

    return-void
.end method

.method public final onMediaConnected(Lru/ok/android/externcalls/sdk/connection/MediaConnectionListener$ConnectedInfo;)V
    .locals 0

    invoke-virtual {p1}, Lru/ok/android/externcalls/sdk/connection/MediaConnectionListener$ConnectedInfo;->isFirstConnection()Z

    move-result p1

    if-nez p1, :cond_0

    return-void

    :cond_0
    invoke-virtual {p0}, Lcra;->d()V

    return-void
.end method

.method public final onMediaDisconnected(Lru/ok/android/externcalls/sdk/connection/MediaConnectionListener$DisconnectedInfo;)V
    .locals 0

    return-void
.end method

.method public final onParticipantsAdded(Ljava/util/List;)V
    .locals 0

    invoke-virtual {p0}, Lcra;->d()V

    return-void
.end method

.method public final onParticipantsChanged(Ljava/util/List;)V
    .locals 0

    invoke-virtual {p0}, Lcra;->d()V

    return-void
.end method

.method public final onParticipantsRemoved(Ljava/util/List;)V
    .locals 0

    invoke-virtual {p0}, Lcra;->d()V

    return-void
.end method

.method public final onParticipantsUpdated(Ljava/util/Collection;)V
    .locals 0

    invoke-virtual {p0}, Lcra;->d()V

    return-void
.end method

.method public final onRolesChanged(Lru/ok/android/externcalls/sdk/ConversationParticipant;)V
    .locals 0

    invoke-virtual {p0}, Lcra;->d()V

    return-void
.end method
